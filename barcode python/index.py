...
def main():
    global oldString
    camera = cv2.VideoCapture("https://192.168.10.2:8080/video")
    ret, frame = camera.read()
    #2
    while ret:
        ret, frame = camera.read()
        barcodeString = read_barcodes(frame)
        # print(len(barcodeString))
        if (len(barcodeString) == 50 or len(barcodeString) == 30 or len(barcodeString) == 16) and barcodeString != oldString:
          oldString = barcodeString
